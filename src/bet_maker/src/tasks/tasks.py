...
@celery_app.task
def update_event_state(bet_id: BetIDSchema, event_state: BetStateSchema):
    """The task for update event state on bet"""

    current_loop = asyncio.get_event_loop()
    current_loop.run_until_complete(
        update_event_state_async(bet_id, event_state)
    )
...
async def update_completed_event_state_async(
    ...
    for bet_id in bets_id:
        update_event_state.delay(bet_id, event_state)


async def update_event_state_async(
    bet_id: BetIDSchema, event_state: BetStateSchema
):
